...
export async function PUT(request, { params }) {
  try {
    ...
    const existingAccount = await prisma.accountsPayable.findUnique({
      where: { id: params.id },
      include: {
        purchaseOrder: true,
      },
    ...
    if (existingAccount.purchaseOrderId && data.amount !== undefined) {
      const orderTotal = Number(existingAccount.purchaseOrder.total)
      const otherAccountsTotal = await prisma.accountsPayable.aggregate({
        where: {
          purchaseOrderId: existingAccount.purchaseOrderId,
          id: { not: params.id },
          status: 'OPEN',
        },
        _sum: {
      ...
      if (newTotal > orderTotal + 0.01) {
        ...
        return NextResponse.json(
          { 
            success: false, 
            error: { 
              message: `O valor total das contas a pagar (R$ ${newTotal.toFixed(2)}) excede o total do pedido (R$ ${orderTotal.toFixed(2)}). Já existem outras contas a pagar no valor de R$ ${otherTotal.toFixed(2)}. Valor disponível: R$ ${available > 0 ? available.toFixed(2) : '0,00'}`, 
              code: 'BAD_REQUEST' 
            } 
          },
          { status: 400 }
...
export async function DELETE(request, { params }) {
  try {
    ...
    // Não permitir exclusão se a conta já foi paga
    if (existingAccount.status === 'PAID') {
      return NextResponse.json(
        { success: false, error: { message: 'Não é possível excluir uma conta já paga', code: 'BAD_REQUEST' } },
        { status: 400 }
      )
    }
