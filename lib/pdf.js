...
export function generateSalesOrderPDF(salesOrder) {
  ...
  if (salesOrder.accountsReceivable && salesOrder.accountsReceivable.length > 0) {
    ...
    salesOrder.accountsReceivable.forEach((ar) => {
      ...
      yPosition += 4
      yPosition = addText(`Vencimento: ${dueDate} | Status: ${status} | Valor: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(amount)}`, margin + 5, yPosition, { fontSize: 8 })
      yPosition += 6
    })
  }
      ...
      html += `
        <div class="account-item">
          ...
          <div style="margin-top: 5px; font-size: 14px;">
            Vencimento: ${dueDate} | Status: ${status} | Valor: ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(amount)}
          </div>
        </div>
      `
    })
